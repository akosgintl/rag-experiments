...
class MarkItDownAdvancedProcessor:
    ...
    def extract_comprehensive_content(self, docx_path: str) -> Dict[str, Any]:
        ...
        """
        try:
            print(f"Processing {docx_path} with MarkItDown...")
            ...
            extracted_content = {
                ...
                "metadata": {
                    "source": docx_path,
                    "title": result.title or Path(docx_path).stem,
            ...
            md_path = self.output_dir / "text" / f"{Path(docx_path).stem}_markdown.md"
        ...
        for match in table_matches:
            ...
            try:
                ...
                if headers and rows:
                    ...
                    # Save original markdown table
                    md_path = self.output_dir / "tables" / f"{Path(docx_path).stem}_table_{table_counter}.md"
                    with open(md_path, 'w', encoding='utf-8') as f:
    ...
    def create_comprehensive_chunks(self, extracted_content: Dict, docx_path: str) -> List[Dict[str, Any]]:
        ...
        if has_headers:
            ...
            chunks = self._chunk_by_headers(markdown_content, docx_path)
        else:
    ...
    def _chunk_by_headers(self, markdown_content: str, docx_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, chunk in enumerate(header_chunks):
                if len(chunk.page_content) > 1500:
                    ...
                    for j, sub_chunk in enumerate(sub_chunks):
                        chunk_obj = {
                            "id": f"{Path(docx_path).stem}_header_chunk_{i}_{j}",
                            "type": "text",
                            "content": sub_chunk,
                    ...
                    chunk_obj = {
                        "id": f"{Path(docx_path).stem}_header_chunk_{i}",
                        "type": "text", 
            ...
            return self._chunk_by_characters(markdown_content, docx_path)
    
    def _chunk_by_characters(self, markdown_content: str, docx_path: str) -> List[Dict[str, Any]]:
        ...
        for i, chunk in enumerate(text_chunks):
            chunk_obj = {
                "id": f"{Path(docx_path).stem}_char_chunk_{i}",
                "type": "text",
                "content": chunk,
...
#    collection_name = f"markitdown_comprehensive_{Path(docx_path).stem}"
